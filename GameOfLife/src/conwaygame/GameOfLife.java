    ...
    */
    public GameOfLife (String file) {
        ...
        for(int i=0 ; i<r ; i++)
        {
            for (int j=0 ; j<c ; j++)
            {
                boolean isAliveDead = StdIn.readBoolean();
                grid[i][j] = isAliveDead;
    ...
    public int numOfAliveNeighbors (int row, int col) {
        ...
        {
            int RowFinal = (row + r[i] + grid.length) % grid.length;
            int ColFinal = (col + c[i] + grid[0].length) % grid[0].length;

            if(grid[RowFinal][ColFinal])
            {
                aliveNeighbors++;
            }
        }
    ...
    public int numOfCommunities() {
        ...
        for(int i=0 ; i<grid.length ; i++)
        {
            for(int j=0 ; j<grid[0].length ; j++)
            {
                if(grid[i][j])
                {
                    ...
                    {
                        int RowFinal = (i + r[k] + grid.length) % grid.length;
                        int ColFinal = (j + c[k] + grid[0].length) % grid[0].length;

                        if(grid[RowFinal][ColFinal])
                        {
                            wuf.union(i, j, RowFinal, ColFinal);
                        }   
                    }
                }
            }
        ...
        int numCommunites = 0;
        boolean[] arrRoots = new boolean[grid.length*grid[0].length];
        ...
        for(int i=0 ; i<grid.length ; i++)
        {
            for(int j=0 ; j<grid[0].length ; j++)
            {
                if(grid[i][j])
                {
                    int root = wuf.find(i, j);
                    arrRoots[root] = true;
                }
            }
        }

        for(boolean a : arrRoots)
        {
            if(a)
            {
                numCommunites++;
        ...
        return numCommunites;// update this line, provided so that code compiles
    }
}
