...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for(int i=0 ; i<categoryVar.length ; i++)
        {
            ...
            StdIn.readLine(); //typical scanner error can't read int after reading string, so moving it to next line

            for(int j=0 ; j<numDishes ; j++)
            {
                String dishName = StdIn.readLine();//reads knight nachos
                int numIngredientId = StdIn.readInt();//reads 9
                int[] stockID = new int[numIngredientId];

                for(int k=0 ; k < numIngredientId ; k++)
                {
                    stockID[k] = StdIn.readInt();
                }
                StdIn.readLine();//typical scanner error can't read int after reading string, so moving it to next line
                Dish dish = new Dish(category , dishName , stockID);
                MenuNode menuNode = new MenuNode(dish , null);

                if(menuVar[i] == null) //if menuVar is empty then add menuNode to index i
                {
                    menuVar[i] = menuNode;
                }
                else
                {
                    menuNode.setNextMenuNode(menuVar[i]); //set the current node to next node
                    menuVar[i] = menuNode; //and add the menuNode at index i
                }
    ...
    public void addStockNode ( StockNode newNode ) {
	    ...
	    // WRITE YOUR CODE HERE
        int ingredientId = newNode.getIngredient().getID(); //gets ingredient node from newNode and then gets ID from Ingredienet Node
        ...
        {
            newNode.setNextStockNode(stockVar[index]); //make the already present node the next node
            stockVar[index] = newNode; //set the newNode at the index
        }
    }
    ...
    public StockNode findStockNode (int ingredientID) {
	    ...
	    // WRITE YOUR CODE HERE
        int hashKey = ingredientID % stockVarSize;
        StockNode ptr = stockVar[hashKey];
        while(ptr != null)
        {
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        //update sotck Amount
        int currentAmount = ptr.getIngredient().getStockLevel() + stockAmountToAdd;
        ptr.getIngredient().setStockLevel(currentAmount);
    }
    ...
    public void updatePriceAndProfit() {
        ...
        {
            MenuNode menuPtr = menuVar[i];
            while(menuPtr != null)
            {
                int[] stockId = menuPtr.getDish().getStockID();
                double cost = 0;
                for(int j=0 ; j<stockId.length ; j++)
                {
                    cost += findStockNode(stockId[j]).getIngredient().getCost();
                }
                double price = cost*1.2;
                menuPtr.getDish().setPriceOfDish(price);

                double profit = price - cost;
                menuPtr.getDish().setProfit(profit);

                menuPtr = menuPtr.getNextMenuNode();
            }
        }
    }
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        int[] stockId = menuNode.getDish().getStockID();
        for(int i : stockId)
        {
            if(findStockNode(i).getIngredient().getStockLevel() < numberOfDishes)
            {
                available = false;
                break;
            }
        ...
        return available; // update the return value
    }
    ...
    public void order (String dishName, int quantity){
	    ...
	    // WRITE YOUR CODE HERE
        TransactionData tData; //initializing Transaction Data
        
        if(checkDishAvailability(dishName, quantity))
        {
            double profit = quantity * findDish(dishName).getDish().getProfit(); //quantity * profit per dish
            tData = new TransactionData("order" , dishName , quantity , profit , true);
            addTransactionNode(tData);
            int[] stockId = findDish(dishName).getDish().getStockID(); //getting the ingredients ID used to make dish
            for(int x : stockId)
            {
                updateStock(null, x, -quantity);
            }
        }
        else
        {
            tData = new TransactionData("order" , dishName , quantity , 0 , false);
            addTransactionNode(tData);
            
            MenuNode menuNode = findDish(dishName).getNextMenuNode(); //starting from the next dish that user picked
            
            if(menuNode == null)
            {   
                menuNode = menuVar[dishIndexFinder(dishName)];//setting current menuNode to first menuNode
            }
     
            while(!menuNode.getDish().getName().equals(dishName))
            {

                if(checkDishAvailability(menuNode.getDish().getName(), quantity))
                {
                    double profit = quantity * menuNode.getDish().getProfit(); //quantity * profit per dish
                    tData = new TransactionData("order" , menuNode.getDish().getName() , quantity , profit , true);
                    addTransactionNode(tData);
                    int[] stockId = menuNode.getDish().getStockID(); //getting the ingredients ID used to make dish
                    for(int x : stockId)
                    {
                        updateStock(null, x, -quantity);
                    }
                ...
                tData = new TransactionData("order" , menuNode.getDish().getName() , quantity , 0.0 , false);
                addTransactionNode(tData);

                menuNode = menuNode.getNextMenuNode();
                if(menuNode == null)
                {
                    menuNode = menuVar[dishIndexFinder(dishName)];
                }
            }
        }  
    }

    private int dishIndexFinder(String dishName)
    {
        int index = 0;
        for(int i=0 ; i<menuVar.length ; i++)
        {
	...
	// WRITE YOUR CODE HERE
        double profit = 0.0;
        TransactionNode tNode = transactionVar;
        while(tNode != null)
        {
            profit += tNode.getData().getProfit();
            tNode = tNode.getNext();
        }
    ...
    public void donation (String ingredientName, int quantity){
        ...
        {
            tData = new TransactionData("donation" , ingredientName , quantity , 0.0 , true);
            updateStock(ingredientName, -1, -quantity);
        }
        else
        {
            tData = new TransactionData("donation" , ingredientName , quantity , 0.0 , false);
        }
        addTransactionNode(tData);
    }
    ...
    public void restock (String ingredientName, int quantity){
        ...
        TransactionData tData;
        double profit = profit();
        double cost = quantity * findStockNode(ingredientName).getIngredient().getCost();
        if(profit >= cost)
        {
            tData = new TransactionData("restock" , ingredientName , quantity , -cost , true);
            updateStock(ingredientName, -1, quantity);
        }
        else
        {
            tData = new TransactionData("restock" , ingredientName , quantity , 0.0 , false);
        }
        addTransactionNode(tData);
    }
